...
void LevelA::update(float deltaTime) {
  ...
  mGameState.mouse->update(deltaTime,      // delta time / fixed timestep
                           nullptr,        // player
                           mGameState.map, // map
  ...
  Vector2 currentPlayerPosition = {mGameState.mouse->getPosition().x,
                                   mOrigin.y};
  ...
  } else if (get_num_alive() == 0 && key->getActive() == INACTIVE &&
             !door->getDeadOrAlive()) {
...
void LevelA::render() {
  ...
  Rectangle src = {0.0f, 0.0f, (float)mCurrentBg.width,
                   (float)mCurrentBg.height};
  Rectangle dst = {mOrigin.x - mapWidth / 2.0f, mOrigin.y - mapHeight / 2.0f,
                   mapWidth, mapHeight};
...
void LevelA::initialise() {
  ...
  /*
     ----------- MAP -----------
  */
  mRealMap = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, (unsigned int *)mLevelData1,
                     "assets/cave_/cave.png", TILE_DIMENSION, 5, 5, mOrigin);
  mGhostMap = new Map(LEVEL_WIDTH, LEVEL_HEIGHT, (unsigned int *)mLevelData2,
                      "assets/ghost_tiles.png", TILE_DIMENSION, 4, 4, mOrigin);
  ...
  */
  ...
  key = new Entity({mOrigin.x + 300.0f, mOrigin.y - 20.0f}, // position
                   {50.0f, 50.0f * sizeRatio},              // scale
                   "assets/key.png", // texture file address
                   KEY);
  door = new Entity({mOrigin.x + 300.0f, mOrigin.y - 20.0f}, // position
                    {50.0f, 50.0f * sizeRatio},              // scale
  ...
  */
  ...
  mGameState.camera.target =
      mGameState.mouse->getPosition(); // camera follows player
  mGameState.camera.offset = mOrigin;  // camera offset to center of screen
  mGameState.camera.rotation = 0.0f;   // no rotation
  mGameState.camera.zoom = 1.0f;       // default zoom
  // Key and door start active in the REAL world; they will be ignored in GHOST
  // world logic
...
void LevelA::initialise_enemies() {
  // Spawn enemies at fixed positions based on level origin (away from mouse
  // start)
  real_enemy1 = new Entity({mOrigin.x + 200.0f, mOrigin.y - 200.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/slime.png", // texture file address
                           NPC);
  real_enemy2 = new Entity({mOrigin.x - 100.0f, mOrigin.y}, // position
                           {50.0f, 50.0f},                  // scale
                           "assets/slime.png", // texture file address
                           NPC);
  real_enemy3 = new Entity({mOrigin.x, mOrigin.y + 150.0f}, // position
                           {50.0f, 50.0f},                  // scale
                           "assets/slime.png", // texture file address
                           NPC);
  real_enemy4 = new Entity({mOrigin.x + 150.0f, mOrigin.y - 100.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/slime.png", // texture file address
                           NPC);
  real_enemy5 = new Entity({mOrigin.x + 100.0f, mOrigin.y - 150.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/slime.png", // texture file address
                           NPC);
  dead_enemy1 = new Entity({mOrigin.x + 220.0f, mOrigin.y - 200.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/ghost_slime.png", // texture file address
                           NPC);
  dead_enemy2 = new Entity({mOrigin.x - 120.0f, mOrigin.y + 150.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/ghost_slime.png", // texture file address
                           NPC);
  dead_enemy3 = new Entity({mOrigin.x + 170.0f, mOrigin.y + 120.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/ghost_slime.png", // texture file address
                           NPC);
  dead_enemy4 = new Entity({mOrigin.x + 270.0f, mOrigin.y - 220.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/ghost_slime.png", // texture file address
                           NPC);
  dead_enemy5 = new Entity({mOrigin.x + 220.0f, mOrigin.y - 170.0f}, // position
                           {50.0f, 50.0f},                           // scale
                           "assets/ghost_slime.png", // texture file address
                           NPC);
  key = new Entity({mOrigin.x + 350.0f, mOrigin.y - 100.0f}, // position
                   {50.0f, 50.0f},                           // scale
                   "assets/key.png", // texture file address
                   KEY);
  door = new Entity({mOrigin.x - 350.0f, mOrigin.y + 50.0f}, // position
                    {50.0f, 50.0f},                          // scale
                    "assets/lock.png", // texture file address
                    DOOR);
  real_enemy1->setAIType(FOLLOWER);
  real_enemy1->setAIState(FOLLOWING);
  real_enemy2->setAIType(FOLLOWER);
  real_enemy2->setAIState(FOLLOWING);
  real_enemy3->setAIType(WANDERER);
  real_enemy4->setAIType(FOLLOWER);
  real_enemy4->setAIState(FOLLOWING);
  real_enemy5->setAIType(FOLLOWER);
  real_enemy5->setAIState(FOLLOWING);

  dead_enemy1->setAIType(FOLLOWER);
  dead_enemy1->setAIState(FOLLOWING);
  dead_enemy2->setAIType(FOLLOWER);
  dead_enemy2->setAIState(FOLLOWING);
  dead_enemy3->setAIType(FOLLOWER);
  dead_enemy3->setAIState(IDLE);
  dead_enemy4->setAIType(FOLLOWER);
  dead_enemy4->setAIState(FOLLOWING);
  dead_enemy5->setAIType(FOLLOWER);
  dead_enemy5->setAIState(FOLLOWING);
  ...
  real_enemy1->setColliderDimensions(
      {real_enemy1->getScale().x / 3, real_enemy1->getScale().y / 3});
  real_enemy2->setColliderDimensions(
      {real_enemy2->getScale().x / 3, real_enemy2->getScale().y / 3});
  real_enemy3->setColliderDimensions(
      {real_enemy3->getScale().x / 3, real_enemy3->getScale().y / 3});
  real_enemy4->setColliderDimensions(
      {real_enemy4->getScale().x / 3, real_enemy4->getScale().y / 3});
  real_enemy5->setColliderDimensions(
      {real_enemy5->getScale().x / 3, real_enemy5->getScale().y / 3});

  dead_enemy1->setColliderDimensions(
      {dead_enemy1->getScale().x / 3, dead_enemy1->getScale().y / 3});
  dead_enemy2->setColliderDimensions(
      {dead_enemy2->getScale().x / 3, dead_enemy2->getScale().y / 3});
  dead_enemy3->setColliderDimensions(
      {dead_enemy3->getScale().x / 3, dead_enemy3->getScale().y / 3});
  dead_enemy4->setColliderDimensions(
      {dead_enemy4->getScale().x / 3, dead_enemy4->getScale().y / 3});
  dead_enemy5->setColliderDimensions(
      {dead_enemy5->getScale().x / 3, dead_enemy5->getScale().y / 3});
}
...
void LevelA::update_enemies(float deltaTime) {
  real_enemy1->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  real_enemy2->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  real_enemy3->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  real_enemy4->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  real_enemy5->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  dead_enemy1->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  dead_enemy2->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  dead_enemy3->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  dead_enemy4->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
  dead_enemy5->update(deltaTime, mGameState.mouse, mGameState.map, nullptr, 0);
}
...
void LevelA::shutdown() {
  ...
  if (mBgReal.id)
    UnloadTexture(mBgReal);
  if (mBgGhost.id)
    UnloadTexture(mBgGhost);
...
void LevelA::fire_bullet() {
  ...
  const char *tex = (mGameState.world == REAL) ? "assets/mouse_bullet.gif"
                                               : "assets/ghost_bullet.gif";
...
void LevelA::update_bullets(float deltaTime) {
  ...
  for (auto *b : mBullets) {
    ...
    bool hitEnemy = false;
    Entity *enemies[] = {real_enemy1, real_enemy2, real_enemy3, real_enemy4,
                         real_enemy5, dead_enemy1, dead_enemy2, dead_enemy3,
                         dead_enemy4, dead_enemy5};
